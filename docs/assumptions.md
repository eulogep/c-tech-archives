...
| A-004 | Trois niveaux de confidentialité provisoires suffisent au MVP : `PUBLIC`, `INTERNAL` et `CONFIDENTIAL`. | Provisoire | `ConfidentialityLevel` avec contrainte de base de données et politique RBAC centralisée. | Requis avant une mise en production ou une extension des règles d’accès. |
...
## Visibilité des métriques du dashboard

Les six métriques du dashboard sont actuellement filtrées par le même périmètre RBAC que les archives visibles. **À valider avec C-Tech avant une mise en production :** ces compteurs doivent-ils rester fondés uniquement sur le rôle et la confidentialité, être filtrés par service, ou être réservés à certains profils ? Le MVP n’introduit ni ACL de service ni attribution nominative.
